...
getContentType, downloadContentFromMessage } = require('@adiwajshing/baileys');
 ...
 module.exports = async (
   ...
   cron.schedule('0 0 * * *', () => {
    ...
    if (chatmessage.includes(`ajg`) || chatmessage.includes(`Anjing`)){
       sock.sendMessage(from, 
        { text: '*i m here baby why call me? guck guck🐕*' }, 
        { quoted : m })
...
case 'mining': case 'menambang':{
 ...
 case 'owner': case 'creator': {
  ...
  case 'menu2':{  
   ...
   break
   case 'tes':{
   duingz = fs.readFileSync('./musik/loli.mp3')
     m.reply(from, 'hallo', { quoted : m } )
   }
   sock.sendMessage(from, duingz, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4',ptt:true})
   break
   case 'temp':{
...
case 'awoo': case 'neko':{
 ...
 break
  ...
  case 'glass':{
    if (args.length < 1) return m.reply(from, 'Teks?', { quoted : m } )
     if (!isInventoryLimit){ addInventoriLimit(m.sender) }
     if (isLimit < 1) return reply("Limit kamu sudah habis ✏️, silahkan beli dengan cara #buy limit _jumlah_")
     kurangLimit(m.sender, 1)
     reply(`Satu limit terpakai 🌹\nSisa limit kamu : ${getLimit(m.sender)}`)
